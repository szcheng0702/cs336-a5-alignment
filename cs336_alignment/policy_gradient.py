    ...
    for _ in range(n_grpo_steps):
        ...
        for inputs, ground_truths in train_data_loader:
            ...
            for _ in range(epochs_per_rollout_batch):
                for mb_input_ids, mb_labels, mb_response_mask, mb_log_pbs in zip(
                    torch.chunk(
                    ...
                    torch.chunk(
                        batch_response_mask,
                        chunks=n_microbatches_per_rollout_batch,
                        dim=0,
                    ),
                    torch.chunk(
                        old_log_probs_results["log_probs"],
                        chunks=n_microbatches_per_rollout_batch,
                        dim=0,
                    ),
                ):
                ...
                # step for every gradient_accumulation_steps have been done for loss.backward(), which is every batch
                optimizer.step()
                optimizer.zero_grad()
